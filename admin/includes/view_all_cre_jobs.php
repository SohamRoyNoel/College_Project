...
confirmQuery($update_to_delete_status);
  ...
  $query = "SELECT * FROM jobs WHERE id = '{$postValueId}' ";
    ...
    $query = "INSERT INTO jobs(company_id, object,title, date,HRem,HRph,salary,location,targetTo,skill,mineducation,role,type) ";
      ...
      $query .= "VALUES({$company_idp},'{$object1}','{$title1}',now(),'{$hremail1}','{$hrcontact1}','{$salary1}', '{$location1}', '{$target1}', '{$skill1}', '{$education1}', '{$role1}', 'creator') "; 
...
<a class="btn btn-primary" href="cre_jobs.php?source=add_cre_job">Add New</a>
...
<?php 
    ...
    $query = "SELECT * FROM jobs WHERE type = 'creator' ORDER BY id DESC  ";
    ...
    while($row = mysqli_fetch_assoc($select_posts)) {
        ...
        $query = "SELECT * FROM creators WHERE id = {$company_idp} ";
        ...
        echo "<td><a onClick=\"javascript: return confirm('Are you sure you want to delete'); \" href='cre_jobs.php?delete={$id1}'><i class='fa fa-trash'></i></a></td>"; 
