...
if(isset($_POST['create_job'])) {
    ...
    $query = "INSERT INTO jobs(company_id, object,title, date,HRem,HRph,salary,location,targetTo,skill,mineducation,role,type,img) ";

    $query.="VALUES({$company_id},'{$object}','{$title}',now(),'{$hremail}','{$hrph}','{$salary}','{$location}','{$target}','{$skill}','{$education}','{$role}','creator','20.jpg')";
        ...
        <form id="form"  action="" method="post" enctype="multipart/form-data">
            <div class="vali-form">
                <div class="col-md-6 form-group1">
                    ...
                    <input maxlength="100" minlength="5" type="text" placeholder="Title" required="" name="title">
                </div>
            ...
            <div class="col-md-6 form-group1">
                ...
                <input minlength="5" maxlength="30" type="email" placeholder="Email" required=""  name="hremail">
            </div>
            <br>
            <div class="panel-body">
                <label for="company" class="control-label">Creator</label>
                <select name="company_id" id="" class="form-control">
                    <?php

                    $query = "SELECT * FROM creators ";
                    $select_creator = mysqli_query($connection,$query);
                    confirmQuery($select_creator);
                    while($row = mysqli_fetch_assoc($select_creator )) {
                        $id = $row['id'];
                        $name = $row['name'];
